...
\TitleCitation{Mobile Robot Simulation for Unmanned Surface Vehicles in Ocean Environments}
...
\Author{Brian Bingham\,$^{1,*}$, Carlos Ag{\"u}ero\,$^{2}$, Michael McCarrin\,$^{1}$, Joseph Klamo\,$^{1}$, Joshua Malia\,$^{1}$, Woen-Sug Choi\,$^{3}$, Kevin Allen\,$^{4}$, Tyler Lum\,$^{5}$, Marshall Rawson\,$^{4}$ and Rumman Waqar\,$^{6}$}

%\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Brian Bingham, Carlos Ag{\"u}ero, Michael McCarrin, Joseph Klamo, Joshua Malia, Woen-Sug Choi, Kevin Allen\,$^{4}$, Tyler Lum, Marshall Rawson and Rumman Waqar}

% MDPI internal command: Authors, for citation in the left column
\AuthorCitation{Bingham, B. et al.}
% If this is a Chicago style journal: Lastname, Firstname, Firstname Lastname, and Firstname Lastname.

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{$^{1}$Naval Postgraduate School, $^{2}$Open Robotics, $^{3}$Korea Maritime and Ocean University, $^{4}$University of Florida, $^{5}$University of British Columbia, $^{6}$University of London}

% Contact information of the corresponding author
\corres{Correspondence: bbingham@nps.edu}

% Current address and/or shared authorship
\firstnote{Monterey, CA} 
...
% Keywords
\keyword{simulation, robotics, ocean, unmanned surface vehicle, stochastic process} 
...
For generating physically meaningful wind time series simulations it is necessary to take into account the physical dimensions of the spectra. 
...
\label{e:wavefactor}
\end{equation}
For the purposes of this example developed sea condition is simulated with $H_s=H_n$.
...
Once the added mass terms that make up $\bm{M}_{A}$ are chosen, the derivation by \citet{imlay61complete} provides the Coriolis-centripetal added mass terms that result. The Coriolis-centripetal added mass matrix is expressed using the same hydrodynamic derivatives from $\bm{M}_{A}$. The resulting matrix is
\begin{equation}
    \medmath{\bm{C}_{A}(\bm{\nu}_r)=
    \left[ 
    \begin{array}{cccccc}
...
\begin{figure}[hbt!]
  \centering
  \includegraphics[width=0.8\textwidth]{images/robotx_vrx_wamv.png}
