...
Once the added mass terms to include in the model for $\bm{M}_{A}$ are selected, \citet{} has derived the Coriolis-centripetal added mass terms that result. The Coriolis-centripetal added mass matrix is expressed using the same hydrodynamic derivatives from $\bm{M}_{A}$. The resultant matrix is \color{black}
