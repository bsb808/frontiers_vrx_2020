...
[Thinking this might be a separate section - Klamo?]
...
Notes on the scenarios used to generate figures:

\begin{itemize}
\item The motion (heave, pitch roll) is shown as steady-state amplitude against wave height.
\item Seakeeping experiments done in monochromatic sea conditions with \unit[6]{s} period.
\item Manuevering scenario has magnitude of the overall thruster command is 0.75\% which results in approximate equivalent surge velocity.  Each simulation is run for \unit[60]{s}, so the length of each position plot is approximately equal.
\end{itemize}
