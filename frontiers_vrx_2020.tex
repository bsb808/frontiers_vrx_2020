...
\end{split}
\end{equation}
should velocity for Mrb and Crb be relative velocity? see 8.147 in Fossen 2011 (pg 236). where
...
\end{equation}
\color{red}where, according to \citet{fossen11handbook} we can assume $N_{\dot{v}} = Y_{\dot{r}}$ for the \wamv{} since it is a slow moving surface ship.

Once the added mass terms to include in the mass matrix of the model are decided, \citet{imhlay} has derived what the resulting Coriolis-centripetal added mass terms are. The Coriolis-centripetal added mass matrix is expressed using the same hydrodynamic derivatives.\color{black}
...
\begin{multline}\label{e:D_n}
  ...
  \medmath{
    -\left[ 
      \begin{array}{cccccc}
          X_{u|u|}|u_r| & \hspace{-0.25in}0 & 0 & 0 & 0 & 0 \\
          0 & \hspace{-0.25in}Y_{v|v|}|v_r| & 0 & 0 & 0 & 0  \\
          0 & \hspace{-0.25in}0 & 0 & 0 & 0 & 0   \\
          0 & \hspace{-0.25in}0 & 0 & 0 & 0 & 0  \\
          0 & \hspace{-0.25in}0 & 0 & 0 & 0 & 0  \\
          0 & \hspace{-0.25in}0 & 0 & 0 & 0 & N_{r|r|}|r| 
      \end{array} \right].
  }
\end{multline}

\color{red} As discussed in \citet{fossen11handbook}, the linear damping matrix captures predominately two effects, viscous skin-friction damping and potential damping. In the horizontal plane, the viscous component of the linear damping terms are important for vehicles traveling at low speed. In the vertical plane, the potential damping components are important regardless of the vessel speed. This is due to the fact that unlike the horizontal plane, the vertical plane motions have restoring forces or moments and consequently an associated natural frequency of motion.

\citet{fossen11handbook} highlighted the fact that the nonlinear damping matrix, with its cross-flow damping terms, becomes the dominant damping contribution at high speed. Since we want our vehicle model to include a wide range of surface vehicles, we have included these nonlinear damping terms. However, since the \wamv{} is a predominatly low speed vehicle, when we apply our baseline model to it we set the $Y_{v|v|}$ and $N_{r|r|}$ terms to zero. We would also normally set $X_{u|u|}$ to zero as well but since \citet{sarda16station} already experimentally determined the value, there is no extra effort required to keep the term.\color{black}
...
\end{figure}
%
Each simulation was run for \unit[60]{s} resulting in roughly $10$ wave encounters. The amplitude of the motion response in each degree-of-freedom was determined using the last X wave encounters to not be influenced by the initial transients. The roll and pitch response correspond to the left y-axis scale in the figure and the heave response is plotted against the right y-axis scale. Since these cases correspond to head seas, we expect to see an increasing response in pitch and heave and very little response in roll. The figure shows that the pitch and heave amplitude responses of the \wamv{} increased as the wave height increased. Furthermore, the responses show that the \wamv{} is contouring to the passing waves as typically happens when the wavelength is considerably longer than the vehicle length. This means that the pitch angle matches the local slope of the wave and the heave displacement matches the wave amplitude. For the largest wave simulated, the maximum wave slope, given by $ka$, is \unit[6.4]{$^\circ$} and matches the simulated amplitude of the oscillating pitch angle. The heave amplitude response of the \wamv{} for this wave equals \unit[1]{m} which is the wave amplitude. Finally, the roll amplitude response remains very small as the wave height increases. There is a small response since the vehicle does not remain perfect aligned in a head seas condition but yaws slightly since there is no heading controller during these simulations.

Figure~\ref{f:beam_seas} shows the heave, pitch, and roll response of the \wamv{} again but this time for beam seas. The formatting of the figure axes and data is the same as Fig.~\ref{f:head_seas}.%
%
\begin{figure}[h]
  \centering
...
\end{figure}
%
Since this is a beam seas wave environment, we expect to see a vehicle response in roll now instead of pitch while still seeing a heave response. The figure shows that the simulated roll and heave amplitude responses of the \wamv{} increases as the wave height increases and that the vehicle motion is again contouring to the passing wave. The ratio of the pitch amplitude response to the wave slop is again unity as is the ratio of the heave amplitude to wave amplitude. Finally, consistent with a beam sea, the pitch response of the \wamv{} is much smaller than the roll response.

The second set of simulations were a series of manuevering tests involving turning circles. Manuevering scenario has magnitude of the overall thruster command is 0.75\% which results in approximate equivalent surge velocity.  Each simulation is run for \unit[60]{s}, so the distance travelled by the vehicle is approximately equal for each run shown.
\begin{figure}[h]
  \centering
  \includegraphics[width=\SFc\textwidth]{images/2020_08_25_manuevering_000.png}
  \caption{Simulation results for manuevering in calm water.}
  \label{f:manuevering}
\end{figure}
\color{black}
