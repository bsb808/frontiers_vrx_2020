...
figure(1)
clf()
plot(data[:,0],data[:,1],'ko',label='Measurements')
grid(True)
...
#plot(pdata[:,0],zz,'r',linewidth=2,label='GLF Fit +')#'Pos. final')
cmdpos = linspace(0,1,100)
zz = glf(outp[0],outp[1],outp[2],outp[3],outp[4],outp[5],cmdpos)
plot(cmdpos,zz,'r',linewidth=2,label='GLF Fit +')#'Pos. final')
...
# Neg
cmdneg = linspace(-1,0,100)
zz = glf(outn[0],outn[1],outn[2],outn[3],outn[4],outn[5],cmdneg)
plot(cmdneg,zz,'b',linewidth=2,label='GLF Fit -')#'Neg. final')
