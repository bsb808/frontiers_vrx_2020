...
\begin{abstract}
  \section{}
Physical and visual robotic simulation is a critical capability for autonomous vehicles in general and unmanned ocean surface vehicles in particular.  Because of time and cost constraints, it is often infeasible to use real-world experiments alone to evaluate advances in navigation and perception for autonomous ocean vessels across the necessary space, time and environmental extents.  As a complement to on-water testing, simulation allows developers to parallelize longer duration evaluations, with a multitude of environmental conditions, to exercise algorithms and expose flaws in nominal conditions, as well as edge-cases.
...
For self-driving cars and aviation applications, the need for simulation toolsets is well documented, e.g., \citet{fadaie2019state}.  Demonstrating correctness of navigation and perception algorithms, as well as verifying safety constraints can require hundreds of millions of miles of testing \citet{shalevshwartz2018formal}; clearly intractable for hardware-only experimentation.  In these domains, commercial and open-source simulation capabilities are a critical aspect of the rapid development of new applications, e.g., Waymo's Carcraft \citet{etherington2019waymo} and Microsoft's AirSim \citet{shah2017airsim}.


The open source Virtual RobotX (VRX) simulator\footnote{\url{https://github.com/osrf/vrx}} is designed to support the development, testing and evaluation of unmanned surface vehicle autonomy algorithms operating in authentic ocean environments.  VRX extends the Gazebo robot simulator through the addition of new domain-specific elements including environmental models, vessel dynamics representations, sensor emulation and general purpose ocean object models.  The new capabilities featured in the VRX simulator include:
\begin{itemize}
